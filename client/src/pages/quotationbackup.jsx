...
const sashItem = lists.sashes.find((s) => (s.model || "").toUpperCase().includes("3200 SPHANDLESGU"));
    ...
    const totalPerUnit = typologyAmount + lockAmount + railAmount + glassAmount + insideInterlockAmount + meshInterlockAmount+outsideInterlockAmount + finishAmount+hardwareAmount;
...
{mode === "view" && preRevision && (
  ...
  if (!oldRow) {
    return (
      <tr key={`row-added-${idx}`} className="bg-green-50">
        ...
        <td className="border px-2 py-1" colSpan={3}>➕ New row added</td>
      </tr>
    );
  }
  ...
  >
    <Plus size={14} /> Add Product Row
        ...
        <table className="table-fixed min-w-[1000px] border text-xs mb-4">
         <thead className="bg-gray-100">
            <tr>
              ...
              {mode === "edit" && (
                <th className={`${cellCls} w-28`}>Actions</th>
              )}
            </tr>
          </thead>
                      ...
                      {rows.map((r, i) => {
                        ...
                        return (
                          <tr key={i} className="hover:bg-gray-50">
                            ...
                            <td className={cellCls}>
                              {r.rateType === "sqM" ? r.rateSqM : r.rateSqM}
                            </td>
                            {mode === "edit" && (
                                <td className={cellCls}>
                                  <button
      ...
      <div className="flex justify-end mb-6">
        <div className="w-37">
          <label className="font-medium block mb-1" style={FONT}>Discount</label>
          <input
                  type="number"
      ...
      <div className="mt-4 text-right" style={FONT}>
        ...
        <p>Discount: <b>{discountAmt.toFixed(2)}</b></p>
      ...
      {mode === "view" && quotationId && (
        <button
          onClick={() => nav(`/quotation/${quotationId}/mto`, "_blank")}
